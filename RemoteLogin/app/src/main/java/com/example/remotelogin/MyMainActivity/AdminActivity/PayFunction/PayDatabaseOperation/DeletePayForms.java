...
public class DeletePayForms extends AsyncTask<String, Void, Void> {
    final private String TAG = "MYTAG";
    private Context context;

    public DeletePayForms(Context context) {
    ...
    protected Void doInBackground(String... strings) {
        ...
        if (conn != null) {
            String sql = "delete from post_cost where post_cost_id=" + strings[0];
            try {
...
//                Toast.makeText(context.getApplicationContext(), "修改密码失败", Toast.LENGTH_LONG).show();
    ...
    protected void onPostExecute(Void unused) {
        ...
        Toast.makeText(context.getApplicationContext(), "删除缴费记录成功", Toast.LENGTH_LONG).show();
    }
}