...
public class IDChangePayCash extends AsyncTask<String, Void, Void> {
    final private String TAG = "MYTAG";
    private Context context;

    IDChangePayCash(Context context) {
    ...
    protected Void doInBackground(String... strings) {
        ...
        if (conn != null) {
            String sql = "update post_cost set payed=" + strings[1] + ", unpay=unpay-"+strings[1]+" where post_cost_id = " + strings[0];
            try {
...
//                Toast.makeText(context.getApplicationContext(), "修改密码失败", Toast.LENGTH_LONG).show();
    ...
    protected void onPostExecute(Void unused) {
        ...
        Toast.makeText(context.getApplicationContext(), "缴费成功", Toast.LENGTH_LONG).show();
    }
}
